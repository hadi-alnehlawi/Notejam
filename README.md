...
### Technology Stack ###
We choose to run the applicaiton on AWS cloud providers for many factors. Scalabiltiy and flexibility are the most important factors. In the below a list of technology stacks used in the project:
...
* [Terraform](https://www.terraform.io/): Instead of provisioning the infrastrucre manually, terraform is used to provisioning all the resources. Defining infrastrcue as code creates resources, manages existing ones, and destroies those no longer needed.
...
# Architecting #
Cluster
![Alt text](./cluster.jpeg?raw=true "EKS Cluster")
Database Backup
![Alt text](./backup.jpeg?raw=true "Postgres DB Backup $ Retension")
CICD Pipline
![Alt text](./cicd.jpeg?raw=true "Circle CI/CD Piplines")
...
* EventBridge - to scheudle the lambda fucntion to conduct a snapt shot and export it to S3 bucket.
...
```
* One we successfully build the docker image, we need to push it into any contianer registery, ex [docker hub](https://hub.docker.com) and will be used to build the deployment in the next step. 
